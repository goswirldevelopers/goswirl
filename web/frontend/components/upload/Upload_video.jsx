...
export default function Upload_video() {
  ...
  const handleSelectProduct = async ({ selection }) => {
    ...
    for (let index = 0; index < selection.length; index++) {
      console.log(selection[index], "secletion--");
  ...
  const saveVideo = async(e) => {
    ...
    console.log(files, "files data are ")
    if(files.length>0){
    ...
    if (files.length == 0) {
      setLoading(false)
      return showToast('reqired portrait video .mp4 file between 5 to 180 seconds', true)
    }
    ...
    formData.append('video_file', fileData[1])
  ...
  const handleFileDrop = (e) => {
    ...
    videoElement.onloadedmetadata = () => {
      console.log(videoElement.videoWidth, videoElement.duration, "duration")
      if (videoElement.duration < 5 || videoElement.duration > 180) {
        setFiles([])
        return alert('video atleast 5 to 180 second max.')
      }
      else if(videoElement.videoWidth > videoElement.videoHeight){
        setFiles([])
        return alert('Landscape video is not allowed.')
      } else{
        var url = window.URL.createObjectURL(new Blob(e.target.files, {type: e.target.files[0].type}))
        setFileContent(url)
  ...
  const handleRedirectComp = async() => {
    ...
    <Grid>
        <Grid.Cell columnSpan={{xs: 6, sm: 6, md: 6, lg: 8, xl: 8}} >
            ...
            <Layout.AnnotatedSection>
                ...
                <Form.Label>Upload Video <Badge size="small" status="warning">required</Badge></Form.Label>
                  <Form.Control onChange={handleFileDrop} type="file" label="Upload Video" style={{marginBottom:"16px"}}/>
                  
                  <span><b>Note:</b> Max file size 150Mb  |  Length 5 - 180 sec  |  .mp4 supported  | Aspect ration - 9:16</span>
                </Form.Group>
